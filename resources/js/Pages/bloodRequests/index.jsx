...
const index = ({ auth, bloodRequests, cities }) => {
  const [city, setCity] = useState("");
  const [id, setId] = useState("");
  const [status, setStatus] = useState("");

  const bloodRequestsData = bloodRequests.data
  ...
  useEffect(() => {
    if (data.status) {
      put(route("bloodRequests.update", id));
    }
  }, [data.status, id]);
                ...
                <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                  <div className="overflow-auto">
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                      ...
                      <tbody>
                        {bloodRequestsData.map((bloodRequest) => (
                          <tr
                            key={bloodRequest.id}
                          ...
                          >
                            ...
                            >
                              ...
                              {bloodRequest.bank && (
                                <Link
                                  href={route(
                                    "banks.show",
                                    bloodRequest.bank.id
                                  )}
                                >
                                  {bloodRequest.bank.name}
                                </Link>
                              )}
                            </th>
                            ...
                            <td className="px-4 py-3 text-nowrap">
                              {bloodRequest.city.name}
                            </td>
                            <td className="px-4 py-3 text-nowrap">
                              {bloodRequest.location}
                            </td>
                            <td className="px-4 py-3 text-nowrap">
                              {bloodRequest.urgency_level == "normale" ? (
                                <Badge
