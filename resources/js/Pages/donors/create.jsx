...
export default function Register({ auth, cities }) {
  ...
  useEffect(() => {
    setData("password", data.email);
  },[data.email])

  const submit = (e) => {
    e.preventDefault();

    post(route("donor.register"));
  };

  const bloodTypes = BloodTypes;

  return (
    <Authenticated user={auth.user} pageName={"Create New Donor"}>
                  ...
                  <SelectContent>
                    <SelectGroup>
                      {bloodTypes.map((bloodType) => (
                        <SelectItem key={bloodType.name} value={bloodType.name}>
                          {bloodType.name}
                        </SelectItem>
                      ))}
                    </SelectGroup>
